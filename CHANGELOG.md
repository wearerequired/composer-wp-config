...
### Fixed
* Detect WordPress core by the `wordpress-core` package type instead of by the `johnpbloch/wordpress-core` package name. `wp-config.php` is now also generated for other packages shipping WordPress core, such as `roots/wordpress-full`.

## [1.2.0] - 2026-08-26

### Added
* Add support for `_HTTPS` variable to control the scheme of `WP_HOME` when `$_SERVER['SERVER_NAME']` is not set, like WP-CLI. Defaults to `true`.

## [1.1.1] - 2025-08-18

### Fixed
* Add default value to `required_wp_config.env_names` hook.
...
[Unreleased]: https://github.com/wearerequired/composer-wp-config/compare/1.1.1...HEAD
[1.1.1]: https://github.com/wearerequired/composer-wp-config/compare/1.1.0...1.1.1
